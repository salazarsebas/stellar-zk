...
pub async fn run(config_path: &Path, input_path: &Path, _output_path: Option<&Path>) -> Result<()> {
    ...
    let proof_artifacts = backend
        .prove(&project_dir, &build_artifacts, input_path)
        .await?;
